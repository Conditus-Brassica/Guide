...
logger = logging.getLogger("async_logger_asec")
...
class ArticlesSnippetsEmbCrudAgent(ArticlesCrudAgent):
    """
        ...
        Get settings from elasticsearch_settings:
            ...
            elasticsearch_settings.ARTICLES_SNIPPET_EMB_IND - current index, where all the work happens
    """
    ...
    def __init__(self):
        """
        Initialization ArticlesSnippetsEmbCrudAgent.
        """
    ...
    def _process_result(self, json_params: Dict):
        ...
        return {
            ...
            "snippet": json_params["_source"]["snippet"],
            "snippet_vector": json_params["_source"]["snippet_vector"]
        }
    ...
    async def get_all_documents(self):
        ...
        """
        try:
            resp = await self.__client.search(index=elasticsearch_settings.ARTICLES_SNIPPET_EMB_IND, body={
                "query": {
                    "match_all": {}
                },
                "size": 1000
    ...
    async def get_single_document(self, doc_id: str):
        ...
        """
        try:
            resp = await self.__client.get(index=elasticsearch_settings.ARTICLES_SNIPPET_EMB_IND, id=doc_id)
        ...
        try:
            ...
            if resp.body["hits"]["hits"]:
                return [
                    {
                        "id": doc["_id"],
                        "article_name": doc["_source"]["article_name"],
                        "snippet": doc["_source"]["snippet"]
                    }
                    for doc in resp.body["hits"]["hits"]
                ]
            else:
    ...
    async def create_document(self, json_params: Dict):
        ...
        """
        try:
            resp = await self.__client.create(index=elasticsearch_settings.ARTICLES_SNIPPET_EMB_IND, body=json_params,
                                              id=json_params["id"])
    ...
    async def update_document(self, json_params: Dict):
        ...
        """
        try:
            resp = await self.__client.update(index=elasticsearch_settings.ARTICLES_SNIPPET_EMB_IND,
                                              id=json_params['id'],
                                              body={
                                                  "doc": {
                                                      "article_name": json_params["article_name"],
                                                      "snippet": json_params["snippet"],
                                                      "snippet_vector": json_params["snippet_vector"]
                                                  }
                                              })
    ...
    async def delete_document(self, doc_id: str):
        ...
        """
        try:
            resp = await self.__client.delete(index=elasticsearch_settings.ARTICLES_SNIPPET_EMB_IND, id=doc_id)
    ...
    async def create_multiply_documents(self, document_list: List[Dict]):
        ...
        actions = [
            {
                "_op_type": "create",
                "_index": elasticsearch_settings.ARTICLES_SNIPPET_EMB_IND,
                "_id": doc["id"],
